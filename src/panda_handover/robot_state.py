...
class RobotStateCapture:
    ...
    observed_tool_frame: str | None = None
    T_world_observed_tool: np.ndarray | None = None
    ...
    def validate(self) -> None:
        ...
        if (self.observed_tool_frame is None) != (
            self.T_world_observed_tool is None
        ):
            raise ValueError(
                "observed_tool_frame and T_world_observed_tool must be supplied together"
            )
        ...
        if self.T_world_observed_tool is not None:
            observed_transform = np.asarray(self.T_world_observed_tool)
            if observed_transform.shape != (4, 4):
                raise ValueError("T_world_observed_tool must be 4x4")
            if not np.all(np.isfinite(observed_transform)):
                raise ValueError("T_world_observed_tool contains non-finite values")
            if not np.allclose(
                observed_transform[3], [0.0, 0.0, 0.0, 1.0], atol=1e-7
            ):
                raise ValueError(
                    "T_world_observed_tool has an invalid homogeneous last row"
                )
    ...
    def save(self, capture_directory: str | Path, T_world_camera: np.ndarray) -> Path:
        ...
        if self.T_world_observed_tool is not None:
            T_robot_base_observed_tool = (
                np.linalg.inv(self.T_world_robot_base)
                @ np.asarray(self.T_world_observed_tool, dtype=np.float64)
            )
            np.save(
                output / "T_robot_base_observed_tool.npy",
                T_robot_base_observed_tool,
            )
        report = {
            "schema_version": 2,
            ...
            "observed_tool_frame": self.observed_tool_frame,
            ...
            "transforms": {
                ...
                "T_robot_base_tool.npy": (
                    "planning tool frame in robot base; cross-checks Isaac and cuRobo kinematics"
                    if self.T_world_tool is not None
                    else None
                ),
                "T_robot_base_observed_tool.npy": (
                    "rigid-body frame observed directly in Isaac before the "
                    "profiled fixed transform"
                    if self.T_world_observed_tool is not None
                    else None
                ),
            },
