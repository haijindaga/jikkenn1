...
class TrajectoryReplayTests(unittest.TestCase):
    ...
    def test_grasp_lift_replay_uses_dynamic_target_and_physical_fingers(self):
        ...
        self.assertIn("PhysxSchema.PhysxContactReportAPI.Apply", script)
        self.assertIn("get_physx_simulation_interface", script)
        self.assertIn("fixed_joint_target_contacts_for_latest_step", script)
        self.assertIn('contact_sample["left"] and contact_sample["right"]', script)
        self.assertIn("FIXED_JOINT_MIN_FINGER_CLOSURE_TRAVEL_M", script)
        self.assertIn('"fixed_joint_attachment_gate_failed"', script)
        self.assertIn('"fixed_joint_not_stable_before_lift"', script)
        self.assertIn("FIXED_JOINT_POST_CREATE_SETTLE_FRAMES", script)
        ...
        self.assertIn("def clear_target_velocities()", script)
        self.assertIn("target.set_velocities(np.zeros((1, 6)", script)
        self.assertIn("CreateKinematicEnabledAttr().Set(bool(enabled))", script)
        self.assertIn('"physics_joint_created": False', script)
        self.assertIn("def step_world_with_attachment_sync()", script)
        self.assertIn("world.step(render=False, update_fabric=True)", script)
        self.assertIn("world.render()", script)
    ...
    def test_grasp_lift_replay_records_retention_and_explicit_drive_limit(self):
        ...
        self.assertIn('"stiffness_after": after["stiffness"]', script)
        self.assertIn('"damping_before": before["damping"]', script)
        self.assertIn('"damping_after": after["damping"]', script)
        self.assertIn('"max_force": drive.GetMaxForceAttr()', script)
        self.assertIn("drive_attributes[name].Set", script)
